...
function rhythmVariation(part,bars,seed,strength='explore',scaleSteps=[0,2,3,5,7,8,10],root=0){
 ...
 let result=[];const scale=[];for(let n=12;n<116;n++)if(scaleSteps.includes((n-root+120)%12))scale.push(n);
 ...
 for(let bar=0;bar<bars;bar++){
  ...
  const rhythm=choose(patterns),gate=choose(part.id==='pad'?[.6,.85,1]:[.28,.5,.72,.95]);
...
function relateParts(clip,targets,mode='space',locked={}){
 ...
 clip.parts.forEach(p=>{if(!targets.includes(p.id)||locked[p.id])return;
  if(p.id==='bass'&&chords?.events.length){p.events=p.events.map(e=>{const active=chords.events.filter(c=>c.tick<=e.tick&&c.tick+c.dur>e.tick);if(!active.length)return e;const root=Math.min(...active.map(c=>c.midi))%12;return {...e,midi:36+root};});}
...
function guidedVocalParts(segments,ids,root,steps,bpm,offset,duration,mode='follow',seed='vocal'){
 ...
 sounds.forEach(id=>{let events=[];const poly=['chords','pad','harmony'].includes(id),low=id==='bass';
  if(poly||low||mode==='support'){
   ...
   gaps.forEach(([a,b],i)=>{const source=segments.filter(s=>s.end<=a+.02).slice(-1)[0]||segments[0],gap=b-a;for(let n=0;n<Math.min(4,Math.floor(gap/.16));n++){const st=a+.04+n*.16;if(st+.11>b-.02)break;events.push({tick:Math.round((st+offset)*ts),dur:Math.round(.11*ts),midi:pitch(source.midi-7+(n%2?2:0)),vel:82});}});
  }else{
